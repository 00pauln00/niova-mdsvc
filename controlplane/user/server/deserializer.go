...
const (
	baseKeyIndex   = iota // "u"
	userIDIndex           // "<UserId>"
	fieldKeyIndex         // "<field>" or "cap"
	subFieldIndex         // for cap/<service> or other nested fields
	minFieldParts  = 3    // minimum parts for a valid key: u/<id>/<field>
	capServicePart = 3    // index for capability service name
)
...
const (
	...
	capabilityKeyPrefix = "cap" // capability prefix
)
...
func (UserDeserializer) ParseField(entity Entity, parts []string, value []byte) {
	...
	case capabilityKeyPrefix:
		// Parse capability: /u/<id>/cap/<service>
		if len(parts) > capServicePart {
			service := parts[capServicePart]
			var perm uint32
			if len(value) >= 4 {
				perm = binary.BigEndian.Uint32(value)
			}
			user.Capabilities = append(user.Capabilities, userlib.Capability{
				Service: service,
				Perm:    perm,
			})
		}
