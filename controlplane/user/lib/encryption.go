...
func Initialize() error {
	once.Do(func() {
		...
		// keySrc := os.Getenv("AUTH_CLUSTER_ENCRYPTION_KEY")
		// For testing
		keySrc := "81gavMyXh9dEMT7kM7gR+gS79ovzPwyjWmV1VA/TUII"

		if keySrc == "" {
			initErr = errors.New("AUTH_CLUSTER_ENCRYPTION_KEY environment variable is required")
			log.Error("FATAL: AUTH_CLUSTER_ENCRYPTION_KEY not set")
			return
		}
